...
// app/auth/signup/page.tsx - WITH ROLE PERSISTENCE
"use client"

import { useState, useEffect, Suspense } from "react"
import { useRouter, useSearchParams } from "next/navigation"
import { motion } from "framer-motion"
import { Chrome, Github, Mail, Lock, User, Briefcase, Users, Building2, ArrowRight } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
...
function SignUpContent() {
  ...
  // Get role from URL and set active tab
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/auth/signin?role=${activeTab}`)
      } else {
  ...
  const handleOAuth = (provider: string) => {
    window.location.href = `/api/auth/${provider}?role=${activeTab}`
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4">
      ...
      >
        <Card className="border-0 shadow-2xl bg-white/10 backdrop-blur-xl">
          <CardHeader className="text-center">
            <CardTitle className="text-3xl font-bold text-white">Join Jobde</CardTitle>
            <CardDescription className="text-white/60">Start your journey with us</CardDescription>
          </CardHeader>
          
          <CardContent>
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-white/10">
                <TabsTrigger value="CANDIDATE" className="data-[state=active]:bg-blue-600 gap-2">
                  <Users className="h-4 w-4" />
                  Job Seeker
                </TabsTrigger>
                <TabsTrigger value="RECRUITER" className="data-[state=active]:bg-orange-600 gap-2">
                  <Building2 className="h-4 w-4" />
                  Employer
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value={activeTab} className="space-y-4 mt-4">
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  >
                    {isLoading ? "Creating..." : `Sign up as ${activeTab === "CANDIDATE" ? "Job Seeker" : "Employer"}`}
                  </Button>
                </form>
                ...
                <p className="text-center text-white/40 text-sm">
                  Already have an account?{" "}
                  <a href={`/auth/signin?role=${activeTab}`} className="text-blue-400 hover:underline">
                    Sign in
                  </a>
                </p>
              </TabsContent>
...
export default function SignUpPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white" /></div>}>
      <SignUpContent />
    </Suspense>
  )
}